...
//#include "chrome/browser/metrics/metric_event_duration_details.h" //joek: removed - this is used for 
...
bool DidExtensionViewsStopLoading(extensions::ProcessManager* manager) {
	//joek: RenderViewHosts went away - trying to convert this
...
ExtensionUninstallObserver::ExtensionUninstallObserver(
    ...
    : automation_(automation->AsWeakPtr()),
      reply_message_(reply_message),
      id_(id) {
  //joek: these observers were removed... so maybe they are no longer needed
	//tagging with REMOVED_OBSERVERS
  ...
  switch (type) {
    case content::NOTIFICATION_LOAD_STOP:
      ...
      // and all the extension views have stopped loading.
		//joek: REMOVED_OBSERVERS
		//joek: the process_manager() was removed from extension_system, and I'm not convinced that it's actually being called... so pulling it out for now, but we may have to so something with it
      //if (!extension_ ||
...
MetricEventDurationObserver::MetricEventDurationObserver() {
  ...
  //               content::NotificationService::AllSources());
	//joek: we don't care about metrics.
}
...
NTPInfoObserver::NTPInfoObserver(AutomationProvider* automation,
	...
	//joek: changed this around
  //top_sites_ = automation_->profile()->GetTopSites();
	top_sites_ = TopSitesFactory::GetForProfile(automation_->profile());
  if (!top_sites_) {
    AutomationJSONReply(automation_.get(), reply_message_.release())
  ...
  //joek: Removed as the GetPrepopulatePages was removed in 0da3bc0e26f768834feca7a5e1c0c70f4c88e453 - we may have to reintro that, but for now just trying to move along
