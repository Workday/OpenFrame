...
void AutomationProvider::CreateExternalTab(
  ...
  *session_id = -1;
  scoped_refptr<ExternalTabContainer> external_tab_container =
      ExternalTabContainer::Create(this, automation_resource_message_filter_.get());
  ...
  if (AddExternalTab(external_tab_container.get())) {
...
void AutomationProvider::ProcessUnhandledAccelerator(int handle, const MSG& msg) {
...
void AutomationProvider::SetInitialFocus(int handle, bool reverse,
...
void AutomationProvider::ConnectExternalTab(uint64 cookie,
  ...
  *session_id = -1;
  ...
  if (allow && AddExternalTab(external_tab_container.get())) {
...
void AutomationProvider::OnMessageFromExternalHost(int handle,
  ...
  WebContents* view_host = GetViewForTab(handle);
  if (!view_host)
    return;
