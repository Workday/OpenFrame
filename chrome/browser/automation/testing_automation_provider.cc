...
void TestingAutomationProvider::GetTabTitle(int handle,
  ...
  *title_string_size = -1;  // -1 is the error code
  if (tab_tracker_->ContainsHandle(handle)) {
    ...
    if (entry != NULL) {
      *title = base::UTF16ToWide(entry->GetTitleForDisplay(std::string()));
    } else {
      *title = std::wstring();
    }
...
void TestingAutomationProvider::ExecuteJavascript(
  ...
  ExecuteJavascriptInRenderViewFrame(base::WideToUTF16(frame_xpath),
                                     base::WideToUTF16(script),
                                     reply_message,
...
void TestingAutomationProvider::GetBookmarksAsJSON(
  ...
  scoped_refptr<BookmarkStorage> storage(
      new BookmarkStorage(bookmark_model,
...
void GetChildProcessHostInfo(base::ListValue* child_processes) {
  for (BrowserChildProcessHostIterator iter; !iter.Done(); ++iter) {
    ...
    item->SetString(
        "type",
        content::GetProcessTypeNameInEnglish(iter.GetData().process_type));
