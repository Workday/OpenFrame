...
//joek: Had fun rewriting this section
net::UploadData* CreateUploadData(
  ...
  const std::vector<scoped_ptr<net::UploadElementReader>>* element_readers =
	  upload_data_stream->GetElementReaders();
  for (size_t i = 0; i < element_readers->size(); ++i) {
	  const net::UploadElementReader* reader = (*element_readers)[i].get();
  ...
  //upload_data->set_last_chunk_appended(upload_data_stream->last_chunk_appended()); //joek: removed because updatedatastream was split into multiple files and the last chunk stuff is never called now... so hopefully don't need this
  return upload_data;
}
  ...
  if (!is_protocol_factory_registered_) {
    ...
    old_https_factory_ =
        net::URLRequest::Deprecated::RegisterProtocolFactory(
			...
			*/
	  //joek: When this was removed 0d1e12729790d542b7a9d66afa83916e3fbbb481 the couple of locations were just flatout removed - maybe this isn't needed anymore
...
net::URLRequestJob* URLRequestAutomationJob::Factory(
  ...
  if (scheme_is_http || scheme_is_https) {
    const content::ResourceRequestInfoImpl* info = content::ResourceRequestInfoImpl::ForRequest(request);
    if (info) {
...
void URLRequestAutomationJob::GetResponseInfo(net::HttpResponseInfo* info) {
  ...
  if (request_->url().SchemeIsCryptographic()) { //joek: Changed this
...
void URLRequestAutomationJob::OnMessage(const IPC::Message& message) {
  ...
  //joek: changed the following from IPC_BEGIN_MESSAGE_MAP_EX
  IPC_BEGIN_MESSAGE_MAP(URLRequestAutomationJob,
                           message)
...
void URLRequestAutomationJob::OnDataAvailable(
  ...
  if (pending_buf_.get() && pending_buf_->data()) {
    ...
    //NotifyReadComplete(bytes_to_copy);//joek: this was removed in a505cbb45ec92d2617fe80490acf7d01ef4d3225 - might have to revisit as the contract was changed
  } else {
...
void URLRequestAutomationJob::OnRequestEnd(
  ...
  if (!is_done()) {
    ...
    if (!has_response_started()) {
      ...
      pending_buf_ = NULL;
      pending_buf_size_ = 0;
      //NotifyDone(status); //joek: this was removed in a505cbb45ec92d2617fe80490acf7d01ef4d3225 - might have to revisit as the contract was changed
	  //NotifyReadComplete(0); //joek: this was removed in a505cbb45ec92d2617fe80490acf7d01ef4d3225 - might have to revisit as the contract was changed
    } else {
...
void URLRequestAutomationJob::StartAsync() {
  ...
  if (referrer.SchemeIsCryptographic() && !request_->url().SchemeIsCryptographic()) { //joek: changed this
...
void URLRequestAutomationJob::NotifyJobCompletionTask() {
  if (!is_done()) {
    //NotifyDone(request_status_); //joek: this was removed in a505cbb45ec92d2617fe80490acf7d01ef4d3225 - might have to revisit as the contract was changed
  }
