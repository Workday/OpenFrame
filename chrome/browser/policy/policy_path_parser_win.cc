...
void CheckUserDataDirPolicy(base::FilePath* user_data_dir) {
  ...
  const bool is_chrome_frame =
	  !user_data_dir->empty(); //removed other checks, because we just compile for WOF - not sure if this will stop the standalone from working
  DVLOG(1) << "*** " << __FUNCTION__ << " is_chrome_frame: " << is_chrome_frame;
  ...
  std::wstring key_name(base::ASCIIToUTF16(key_name_ascii));
  if (LoadUserDataDirPolicyFromRegistry(HKEY_LOCAL_MACHINE,
                                        user_data_dir) ||
