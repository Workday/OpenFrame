  ...
  if ((message.type() == ChromeViewHostMsg_GetCookies::ID ||
    ...
    IPC_BEGIN_MESSAGE_MAP(ChromeRenderMessageFilter, message)
      IPC_MESSAGE_HANDLER_DELAY_REPLY(ChromeViewHostMsg_GetCookies,
                                      OnGetCookies)
      //IPC_MESSAGE_HANDLER(ChromeViewHostMsg_SetCookie, OnSetCookie) //joek: 
    IPC_END_MESSAGE_MAP()
    handled = true;
  }
#endif
...
}*/ //joek: This may have been a bad merge because it doesn't look like there's a function header
...
void ChromeRenderMessageFilter::OnGetCookies(
    ...
    IPC::Message* reply_msg) {
#if defined(ENABLE_AUTOMATION)
	//joek: If this is called, it will fail.  request_context_ was removed from the constructor - might need to trace that down and replace it.
  //AutomationResourceMessageFilter::GetCookiesForUrl(
  //    this, request_context_.get()->GetURLRequestContext(), render_process_id_,
  //    reply_msg, url);
#endif
}
