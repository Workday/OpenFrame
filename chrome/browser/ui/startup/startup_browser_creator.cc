...
bool StartupBrowserCreator::ProcessCmdLineImpl(
  ...
  if (process_startup &&
    ...
    if (command_line.HasSwitch(switches::kNoStartupWindow) /*&&
        !command_line.HasSwitch(switches::kAutoLaunchAtStartup)*/) { //joek: Removed startup switch (doesn't exist)
  ...
  if (command_line.HasSwitch(switches::kAutomationClientChannelID)) {
    ...
    if (expected_tabs == 0)
      silent_launch = true;
	LOG(INFO) << "$ " << __FUNCTION__ << " about to CreateAutomationProvider";
    if (command_line.HasSwitch(switches::kChromeFrame)) {
#if defined(OS_WIN)
		LOG(INFO) << "$ " << __FUNCTION__ << " creating ChromeFrameAutomationProvider";
      if (!CreateAutomationProvider<ChromeFrameAutomationProvider>(
          automation_channel_id, last_used_profile, expected_tabs))
        return false;
#endif
    } else {
		LOG(INFO) << "$ " << __FUNCTION__ << " creating plain AutomationProvider";
...
bool StartupBrowserCreator::CreateAutomationProvider(
  ...
  LOG(INFO) << "$ " << __FUNCTION__ << " about to initialize channel";
  if (!automation->InitializeChannel(channel_id))
    return false;
  LOG(INFO) << "$ " << __FUNCTION__ << " setting expected tabs " << expected_tabs;
  automation->SetExpectedTabCount(expected_tabs);

  LOG(INFO) << "$ " << __FUNCTION__ << " get automation provider list";
  AutomationProviderList* list = g_browser_process->GetAutomationProviderList();
  DCHECK(list);
  list->AddProvider(automation.get());
  LOG(INFO) << "$ " << __FUNCTION__ << " post add provider - returning true";
#endif  // defined(ENABLE_AUTOMATION)
