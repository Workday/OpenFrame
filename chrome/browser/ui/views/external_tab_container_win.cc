...
scoped_ptr<NativeWebKeyboardEvent> CreateKeyboardEvent(const MSG& msg) {
  ...
  // TODO(grt): confirm that this is a translated character event.
	ui::KeyEvent key_event(msg); //joek: ui::KeyEvent key_event(msg, true);
...
bool ShouldHideReferrer(const GURL& url, const GURL& referrer) {
  bool referrer_is_secure = referrer.SchemeIsCryptographic(); //joek: switching SchemeIsSecure() to SchemeIsCryptographic()
  ...
  return !url.SchemeIsCryptographic(); //joek: switching SchemeIsSecure() to SchemeIsCryptographic()
}
...
class ExternalTabRootWindowHost : public views::DesktopWindowTreeHostWin {
 public:
...
ExternalTabContainerWin::ExternalTabContainerWin(
    ...
    : widget_(NULL),
      ...
      handle_top_level_requests_(false), //joek: pulled to match
...
bool ExternalTabContainerWin::Init(Profile* profile,
                                   ...
                                   bool handle_top_level_requests, //joek: pulled to match
  ...
  handle_top_level_requests_ = handle_top_level_requests; //joek: pulled to match
  ...
  LOG(INFO) << "@@Questionable " << __FUNCTION__ << " " << handle_top_level_requests;
  ...
  //joek: pulling this out, but we have problems as render_frame_impl removed this and I'm just not sure if CF needs this functionality to work
  ...
  TabHelpers::AttachTabHelpers(existing_contents);
...
HWND ExternalTabContainerWin::GetContentHWND() const {
  return views::HWNDForNativeWindow(web_contents_->GetNativeView());
}
  ...
  switch (params.disposition) {
    ...
    case SAVE_TO_DISK:
      if (automation_) {
        ...
        nav_params.transition = ui::PAGE_TRANSITION_LINK;
        ...
        const history::HistoryAddPageArgs& add_page_args =
            history_tab_helper->CreateHistoryAddPageArgs(
				...
				false /* did_replace_entry */, last_committed->GetUniqueID(), nav_params);
...
void ExternalTabContainerWin::AddNewContents(WebContents* source,
  ...
  if (!route_all_top_level_navigations_) {
    ...
    new_container = new TemporaryPopupExternalTabContainerWin(
        automation_.get(), automation_resource_message_filter_.get());
  ...
  bool result = new_container->Init(profile,
                                    ...
                                    handle_top_level_requests_, //joek: pulled to match
                                    new_contents,
                                    GURL(),
                                    GURL(),
                                    true,
...
void ExternalTabContainerWin::WebContentsFocused(
  ...
  LOG(INFO) << "@@Questionable " << __FUNCTION__;
  //tab_contents_container_->OnWebContentsFocused(contents); //joek: removed because this method is now protected...
}
...
void ExternalTabContainerWin::RegisterRenderViewHostForAutomation(
  ...
  if (!GetTabHandle()) {
    ...
    AutomationResourceMessageFilter::RegisterRenderView(
        ...
        GetTabHandle(),
        automation_resource_message_filter_.get(),
        pending_view);
  }
}
...
bool ExternalTabContainerWin::HandleContextMenu(
  ...
  external_context_menu_.reset(RenderViewContextMenuViews::Create(
      web_contents()->GetFocusedFrame(), params));
  static_cast<RenderViewContextMenuViews*>(
...
void ExternalTabContainerWin::RequestMediaAccessPermission(
	...
	LOG(INFO) << "@@Questionable " << __FUNCTION__;
  //MediaStreamInfoBarDelegate::Create(web_contents, request, callback); //joek: Removed this, as it was converted to 'android' -- not sure if we'll hit this line of code
}
...
void ExternalTabContainerWin::Observe(
  ...
  switch (type) {
    case content::NOTIFICATION_LOAD_STOP: {
        ...
        if (load && ui::PageTransitionIsMainFrame(load->origin)) {
...
bool ExternalTabContainerWin::ProcessUnhandledKeyStroke(HWND window,
  ...
  if ((wparam == VK_TAB) && !ui::win::IsCtrlPressed()) {
...
bool ExternalTabContainerWin::AcceleratorPressed(
  ...
  switch (command_id) {
    ...
    case IDC_DEV_TOOLS:
		LOG(INFO) << "@@Questionable " << __FUNCTION__;
		//joek: *** DANGER - Commented out some features I'm pretty sure we're using at some point in Development
		//However, I have no idea how to get the current browser
		//The function being called is the same as it was a year ago, but the first parameter is no longer accepted
		//joeK: Revisit this
  ...
  if (load_requests_via_automation_) {
    ...
    if (rvh) {
      AutomationResourceMessageFilter::ResumePendingRenderView(
          ...
          tab_handle_, automation_resource_message_filter_.get());
    }
  }
...
void ExternalTabContainerWin::SetupExternalTabView() {
  ...
  infobar_container->ChangeInfoBarManager(
