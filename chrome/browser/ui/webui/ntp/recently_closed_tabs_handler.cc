...
void TabToValue(const sessions::TabRestoreService::Tab& tab,
...
void WindowToValue(const sessions::TabRestoreService::Window& window,
                   base::DictionaryValue* dictionary) {
...
void RecentlyClosedTabsHandler::HandleReopenTab(const base::ListValue* args) {
  ...
  sessions::LiveTabContext* delegate =
...
void RecentlyClosedTabsHandler::CreateRecentlyClosedValues(
  ...
  for (sessions::TabRestoreService::Entries::const_iterator it = entries.begin();
	...
	if (entry->type == sessions::TabRestoreService::TAB) {
      TabToValue(*static_cast<sessions::TabRestoreService::Tab*>(entry),
                 entry_dict.get());
    } else  {
      DCHECK_EQ(entry->type, sessions::TabRestoreService::WINDOW);
	  WindowToValue(*static_cast<sessions::TabRestoreService::Window*>(entry),
                    entry_dict.get());
    }
