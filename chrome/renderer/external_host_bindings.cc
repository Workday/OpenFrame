...
ExternalHostBindings::ExternalHostBindings(IPC::Sender* sender, int routing_id)
    : frame_(NULL), sender_(sender),on_message_handler_(0) {
}

ExternalHostBindings::~ExternalHostBindings() {
}

gin::WrapperInfo ExternalHostBindings::kWrapperInfo = {
    gin::kEmbedderNativeGin};

gin::ObjectTemplateBuilder ExternalHostBindings::GetObjectTemplateBuilder(
    v8::Isolate* isolate) {
		DLOG(INFO) << "***GetObjectTemplateBuilder";
  return gin::Wrappable<ExternalHostBindings>::GetObjectTemplateBuilder(isolate)
      //.SetMethod("postMessage", &ExternalHostBindings::PostMessage)
      //.SetProperty("onmessage", &on_message_handler_)
      ;
}

void ExternalHostBindings::PostMessage(/*const CppArgumentList& args, CppVariant* result*/) {
  /*DCHECK(result);
...
bool ExternalHostBindings::ForwardMessageFromExternalHost(
  ...
  DLOG(INFO) << "***ForwardMessageFromExternalHost";
  bool status = false;
...
void ExternalHostBindings::BindToJavascript(blink::WebFrame* frame,
  ...
  frame_ = frame;
  //joek: see comment in class header
  //CppBoundClass::BindToJavascript(frame, classname);
  /*NPObject* np_obj = WebBindings::createObject(
	  npp_.get(),
	  &CppNPObject::np_class_);
  frame->bindToWindowObject(base::ASCIIToUTF16(classname), np_obj);*/
  DLOG(INFO) << "***BindToJavascript " << classname;
}
...
bool ExternalHostBindings::CreateMessageEvent(NPObject** message_event) {
  ...
  DCHECK(NPVARIANT_IS_OBJECT(document));
