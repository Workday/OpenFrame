...
#include "components/version_info/version_info_values.h.version"
...
class AutomationMessageFilter : public IPC::MessageFilter {
 public:
  explicit AutomationMessageFilter(AutomationProxy* server) : server_(server) {DVLOG(1) << "* " << __FUNCTION__ << " Created ";}

  // Return true to indicate that the message was handled, or false to let
  // the message be handled in the default way.
  virtual bool OnMessageReceived(const IPC::Message& message) override {
    DVLOG(1) << "* " << __FUNCTION__ << " OnMessageReceived " << &message;
    bool handled = true;
  ...
  virtual void OnChannelError() override {
	DVLOG(1) << "* " << __FUNCTION__ << " Sending Signal";
 ...
 private:
  ...
  void OnAutomationHello(const IPC::Message& hello_message) {
    ...
    base::PickleIterator iter(hello_message);
	if (!iter.ReadString(&server_version)) {
...
void AutomationProxy::InitializeChannel(const std::string& channel_id,
  ...
  DVLOG(1) << "* " << __FUNCTION__ << " Initialize ";
  ...
  // process.
  channel_ = IPC::SyncChannel::Create(this,  // we are the listener
	  thread_->task_runner().get(),
      shutdown_event_.get());
  DVLOG(1) << "* " << __FUNCTION__ << " AddFilter ";
...
AutomationLaunchResult AutomationProxy::WaitForAppLaunch() {
  ...
  DVLOG(1) << "* " << __FUNCTION__ << " Start";
  if (app_launched_.TimedWait(action_timeout_)) {
    DVLOG(1) << "* " << __FUNCTION__ << " Application Launched";
    if (server_version_ == kChannelErrorVersionString) {
      DVLOG(1) << "* " << __FUNCTION__ << " AUTOMATION_CHANNEL_ERROR";
	  ...
	  if (server_version_ != version_info::GetVersionNumber()) {
        DVLOG(1) << "* " << __FUNCTION__ << " Version mismatch expected: " << version_info::GetVersionNumber() << " instead: " << server_version_;
        result = AUTOMATION_VERSION_MISMATCH;
      }
    }
  } else {
	DVLOG(1) << "* " << __FUNCTION__ << " TIMEOUT";
...
void AutomationProxy::SignalAppLaunch(const std::string& version_string) {
  DVLOG(1) << "* " << __FUNCTION__ << " Signal: " << version_string;
...
bool AutomationProxy::Send(IPC::Message* message, int timeout_ms) {
  ...
  bool success = channel_->Send(message);
...
void AutomationProxy::SetChannel(IPC::Sender* sender) {
  if (tracker_.get())
    tracker_->put_channel(sender);
}
