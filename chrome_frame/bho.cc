...
STDMETHODIMP Bho::SetSite(IUnknown* site) {
  ...
  if (site) {
    ...
    if (web_browser2) {
      hr = DispEventAdvise(web_browser2.get(), &DIID_DWebBrowserEvents2);
    ...
    if (g_patch_helper.state() == PatchHelper::PATCH_IBROWSER) {
      ...
      DCHECK(browser_service) << "DoQueryService - SID_SShellBrowser failed."
          << " Site: " << site << " Error: " << hr;
      if (browser_service) {
        g_patch_helper.PatchBrowserService(browser_service.get());
    ...
    //MetricsService::Start(); //joek: remove MetricsService -- we don't care, do we?
    ...
    DispEventUnadvise(web_browser2.get(), &DIID_DWebBrowserEvents2);
    Release();
  }
...
STDMETHODIMP Bho::BeforeNavigate2(IDispatch* dispatch, VARIANT* url,
  ...
  base::win::ScopedComPtr<IBrowserService> browser_service;
  DoQueryService(SID_SShellBrowser, web_browser2.get(), browser_service.Receive());
  if (!browser_service || !CheckForCFNavigation(browser_service.get(), false)) {
...
void ClearDocumentContents(IUnknown* browser) {
  ...
  if (SUCCEEDED(DoQueryService(SID_SWebBrowserApp, browser,
    ...
    if (doc_disp && SUCCEEDED(doc.QueryFrom(doc_disp.get()))) {
...
bool DocumentHasEmbeddedItems(IUnknown* browser) {
  ...
  if (SUCCEEDED(DoQueryService(SID_SWebBrowserApp, browser,
    ...
    if (SUCCEEDED(container.QueryFrom(document.get()))) {
      ...
      if (enumerator) {
        ...
        while (!has_embedded_items &&
          ...
          if (SUCCEEDED(embedded_web_browser2.QueryFrom(unk.get()))) {
            ...
            if (!base::LowerCaseEqualsASCII(location_url_string, "about:blank")) {
...
void Bho::ProcessOptInUrls(IWebBrowser2* browser, BSTR url) {
  ...
  if (IsValidUrlScheme(GURL(current_url), false)) {
	bool cf_protocol = base::StartsWith(current_url, kChromeProtocolPrefix, base::CompareCase::INSENSITIVE_ASCII);
    if (!cf_protocol && IsChrome(RendererTypeForUrl(current_url))) {
      ...
      DCHECK(browser_service) << "DoQueryService - SID_SShellBrowser failed.";
      MarkBrowserOnThreadForCFNavigation(browser_service.get());
    }
  }
}
