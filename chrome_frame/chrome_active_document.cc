...
STDMETHODIMP ChromeActiveDocument::DoVerb(LONG verb,
  ...
  if (doc_site_) {
    ...
    case OLEIVERB_SHOW: {
	  ...
	  doc_host_handler.QueryFrom(doc_site_.get());
...
STDMETHODIMP ChromeActiveDocument::Load(BOOL fully_avalable,
  ...
  if (bind_context) {
    ...
    bind_context->GetObjectParam(SZ_HTML_CLIENTSITE_OBJECTPARAM,
                                 site.Receive());
    if (site)
      client_site.QueryFrom(site.get());
  }

  if (client_site) {
    SetClientSite(client_site.get());
	DoQueryService(IID_INewWindowManager, client_site.get(),
	...
	DoQueryService(SID_SShellBrowser, client_site.get(), browser_service.Receive());
    if (browser_service) {
#ifdef DEBUG
      bool flagged = 
#endif
		  CheckForCFNavigation(browser_service.get(), true);
  ...
  // further inspection.
  bool is_safe_scheme = cf_url.gurl().SchemeIs(url::kAboutScheme) ||
...
STDMETHODIMP ChromeActiveDocument::SaveHistory(IStream* stream) {
  ...
  DWORD written = 0;
  std::wstring url = base::UTF8ToWide(navigation_info_->url.spec());
...
bool ChromeActiveDocument::IsSchemeAllowed(const GURL& url) {
  ...
  if (url.SchemeIs(url::kAboutScheme)) {
    if (base::LowerCaseEqualsASCII(url.spec(), chrome::kAboutPluginsURL))
      return true;
  }
  return false;
}
  ...
  if (DoesVerbUIActivate(verb)) {
    if (!m_bUIActive) {
      ...
      if (in_place_active_object) {
        if (in_place_frame_)
          in_place_frame_->SetActiveObject(in_place_active_object.get(), NULL);
        if (in_place_ui_window)
          in_place_ui_window->SetActiveObject(in_place_active_object.get(), NULL);
      }
    }
  }
...
bool IsFindAccelerator(const MSG& msg) {
  ...
  return msg.message == WM_KEYDOWN && msg.wParam == 'F' &&
         ui::win::IsCtrlPressed() &&
         !(ui::win::IsAltPressed() || ui::win::IsShiftPressed());
}
...
void ChromeActiveDocument::UpdateNavigationState(
  ...
  if (GetConfigBool(true, kEnableBuggyBhoIntercept)) {
    ...
    if (wb2 && buggy_bho::BuggyBhoTls::GetInstance()) {
      buggy_bho::BuggyBhoTls::GetInstance()->PatchBuggyBHOs(wb2.get());
    }
  }
  ...
  if (new_navigation_info.url.is_valid())
    url_.Allocate(base::UTF8ToWide(new_navigation_info.url.spec()).c_str());
  ...
  if (is_internal_navigation) {
    ...
    if (GetIEVersion() == IE_6 &&
      ...
      DoQueryService(SID_SWebBrowserApp, m_spClientSite, wb2.Receive());
      if (wb2)
        BaseActiveX::BringWebBrowserWindowToTop(wb2.get());
    }
...
void ChromeActiveDocument::OnAttachExternalTab(
  ...
  HRESULT hr = S_OK;
  if (popup_manager_) {
    const std::wstring& url_wide = base::UTF8ToWide(params.url.spec());
...
bool ChromeActiveDocument::PreProcessContextMenu(HMENU menu) {
  ...
  EnableMenuItem(menu, IDC_BACK, MF_BYCOMMAND |
	  (SUCCEEDED(travel_log->GetTravelEntry(browser_service.get(), TLOG_BACK,
                                            NULL)) ?
      MF_ENABLED : MF_DISABLED));
  EnableMenuItem(menu, IDC_FORWARD, MF_BYCOMMAND |
	  (SUCCEEDED(travel_log->GetTravelEntry(browser_service.get(), TLOG_FORE,
                                            NULL)) ?
      MF_ENABLED : MF_DISABLED));
...
HRESULT ChromeActiveDocument::IEExec(const GUID* cmd_group_guid,
  ...
  if (in_place_site)
    hr = frame_cmd_target.QueryFrom(in_place_site.get());
...
bool ChromeActiveDocument::LaunchUrl(const ChromeFrameUrl& cf_url,
  ...
  url_.Allocate(base::UTF8ToWide(cf_url.gurl().spec()).c_str());
  ...
  if (launch_params_) {
    return automation_client_->Initialize(this, launch_params_.get());
  } else {
    std::wstring profile = base::UTF8ToWide(cf_url.profile_name());
...
HRESULT ChromeActiveDocument::OnRefreshPage(const GUID* cmd_group_guid,
  ...
  if (tab_proxy) {
    ...
    DCHECK(navigation_info_->url.is_valid());
    ChromeFrameUrl cf_url;
    cf_url.Parse(base::UTF8ToWide(navigation_info_->url.spec()));
...
void ChromeActiveDocument::OnGoToHistoryEntryOffset(int offset) {
  ...
  if (browser_service && travel_log)
    travel_log->Travel(browser_service.get(), offset);
}
