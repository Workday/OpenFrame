...
class TopLevelWindowMapping {
 public:
  ...
  static TopLevelWindowMapping* GetInstance() {
    return base::Singleton<TopLevelWindowMapping>::get();
  }
 ...
 private:
  ...
  friend struct base::DefaultSingletonTraits<TopLevelWindowMapping>;
...
void ChromeFrameActivex::OnMessageFromChromeFrame(const std::string& message,
  ...
  if (target.compare("*") != 0) {
    ...
    if (is_privileged()) {
      ...
      if (SUCCEEDED(CreateDomEvent("message", message, origin,
        ...
        base::win::ScopedBstr target_bstr(base::UTF8ToWide(target).c_str());
		Fire_onprivatemessage(message_event.get(), target_bstr);

		FireEvent(onprivatemessage_, message_event.get(), target_bstr);
      }
    } else {
  ...
  if (SUCCEEDED(CreateDomEvent("message", message, origin,
                               message_event.Receive()))) {
    Fire_onmessage(message_event.get());

	FireEvent(onmessage_, message_event.get());
...
STDMETHODIMP ChromeFrameActivex::Load(IPropertyBag* bag, IErrorLog* error_log) {
  ...
  GetObjectScriptId(obj_element.get(), object_id.Receive());

  base::win::ScopedComPtr<IHTMLElement2> element;
  element.QueryFrom(obj_element.get());
  HRESULT hr = S_OK;
  ...
  for (int i = 0; SUCCEEDED(hr) && i < arraysize(event_props); ++i) {
    ...
    if (SUCCEEDED(bag->Read(prop, value.Receive(), error_log))) {
      if (value.type() != VT_BSTR ||
          FAILED(hr = CreateScriptBlockForEvent(element.get(), object_id,
                                                V_BSTR(value.ptr()), prop))) {
  ...
  if (SUCCEEDED(bag->Read(base::win::ScopedBstr(L"useChromeNetwork"),
    ...
    if (use_chrome_network.type() == VT_BOOL) {
      hr = put_useChromeNetwork(V_BOOL(use_chrome_network.ptr()));
      DCHECK(hr != E_UNEXPECTED);
    }
  }
...
STDMETHODIMP ChromeFrameActivex::put_src(BSTR src) {
  ...
  if (document_url.SchemeIsCryptographic()) { //joek: switching SchemeIsSecure() to SchemeIsCryptographic()
    GURL source_url(src);
	if (!source_url.SchemeIsCryptographic()) { //joek: same as above
...
HRESULT ChromeFrameActivex::CreateScriptBlockForEvent(
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (element) {
	  ...
	  if (SUCCEEDED(hr = script_element.QueryFrom(element.get()))) {
...
HRESULT ChromeFrameActivex::registerBhoIfNeeded() {
  ...
  hr = bho->SetSite(web_browser2.get());
  if (FAILED(hr)) {
  ...
  web_browser2->PutProperty(base::win::ScopedBstr(bho_class_id_as_string),
                            base::win::ScopedVariant(bho.get()));
  return S_OK;
}
