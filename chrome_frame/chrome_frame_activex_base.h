...
END_MSG_MAP()
    ...
    if (cmd == IDC_ABOUT_CHROME_FRAME) {
      ...
      switch (cmd) {
        ...
        case IDS_CONTENT_CONTEXT_SAVELINKAS: {
          ...
          DoFileDownloadInIE(base::UTF8ToWide(url.spec()).c_str());
          return true;
        }
 ...
 protected:
  ...
  void OnLoad(const GURL& url) {
    ...
    InvokeScriptFunction(onload_handler_, url.spec());
  }

  void OnLoadFailed(int error_code, const std::string& url) {
    InvokeScriptFunction(onerror_handler_, url);
  }
  ...
  void OnMessageFromChromeFrame(const std::string& message,
    ...
    if (SUCCEEDED(CreateDomEvent("message", message, origin,
      ...
      event_var.Set(static_cast<IDispatch*>(message_event.get()));
  ...
  LPARAM OnDownloadRequestInHost(UINT message, WPARAM wparam, LPARAM lparam,
    ...
    cf_url.Parse(base::UTF8ToWide(GetDocumentUrl()));
    ...
    if (download_params->moniker) {
      NavigateBrowserToMoniker(
          doc_site_.get(), download_params->moniker.get(),
          base::UTF8ToWide(download_params->request_headers).c_str(),
          download_params->bind_ctx.get(), NULL, download_params->post_data.get(),
          &flags);
    }
  ...
  STDMETHOD(postMessage)(BSTR message, VARIANT target) {
    ...
    base::WideToUTF8(message, ::SysStringLen(message), &utf8_message);
  ...
  STDMETHOD(postPrivateMessage)(BSTR message, BSTR origin, BSTR target) {
    ...
    base::WideToUTF8(message, ::SysStringLen(message), &utf8_message);
    base::WideToUTF8(origin, ::SysStringLen(origin), &utf8_origin);
    base::WideToUTF8(target, ::SysStringLen(target), &utf8_target);
  ...
  HRESULT GetHandlersForEvent(BSTR event_type, EventHandlers** handlers) {
	...
	if (base::LowerCaseEqualsASCII(base::string16(event_type, event_type_end), "message")) {
      *handlers = &onmessage_;
    } else if (base::LowerCaseEqualsASCII(base::string16(event_type, event_type_end), "load")) {
      *handlers = &onload_;
    } else if (base::LowerCaseEqualsASCII(base::string16(event_type, event_type_end), "loaderror")) {
      *handlers = &onloaderror_;
    } else if (base::LowerCaseEqualsASCII(base::string16(event_type, event_type_end),
                                    "readystatechanged")) {
  ...
  HRESULT CreateDomEvent(const std::string& event_type, const std::string& data,
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (ev->Initialize(container.get(), data, origin, event_type)) {
        *event = ev;
      } else {
  ...
  void HostNavigate(const GURL& url_to_open,
    ...
    if (ie_version == IE_6) {
      ...
      if (cf_url.attach_to_external_tab() &&
        ...
        BringWebBrowserWindowToTop(web_browser2.get());
      }
    }
