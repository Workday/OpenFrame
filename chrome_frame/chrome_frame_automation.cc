...
#include "chrome/app/main_dll_loader_win.cc"
...
class ChromeFrameAutomationProxyImpl::TabProxyNotificationMessageFilter
 ...
 public:
  ...
  virtual bool OnMessageReceived(const IPC::Message& message) {
    DVLOG(1) << "## " << __FUNCTION__ << " msg: " << message.type();
    if (message.is_reply())
      return false;
  ...
  virtual void OnChannelError() {
    ...
    for (; iter != tabs_list_.end(); ++iter) {
      ...
      if (tab) {
        LOG(ERROR) << __FUNCTION__ << " About to die";
        tab->OnChannelError();
        tab->Release();
      }
    }
...
class ChromeFrameAutomationProxyImpl::CFMsgDispatcher
 ...
 protected:
  virtual bool HandleMessageType(const IPC::Message& msg,
	  ...
	  LOG(INFO) << "$ " << __FUNCTION__ << " msg: " << context->message_type();
...
ChromeFrameAutomationProxyImpl::ChromeFrameAutomationProxyImpl(
  ...
  DVLOG(1) << "# " << __FUNCTION__ << " Channel ID: " << channel_id;
...
void ChromeFrameAutomationProxyImpl::SendAsAsync(
	...
	LOG(INFO) << "AutomationMsg_CreateExternalTab::ID " << AutomationMsg_CreateExternalTab::ID;
	LOG(INFO) << "AutomationMsg_ConnectExternalTab::ID " << AutomationMsg_ConnectExternalTab::ID;
	LOG(INFO) << "AutomationMsg_BrowserMove::ID " << AutomationMsg_BrowserMove::ID;
	LOG(INFO) << "AutomationMsg_ProcessUnhandledAccelerator::ID " << AutomationMsg_ProcessUnhandledAccelerator::ID;
	LOG(INFO) << "AutomationMsg_ForwardContextMenuCommandToChrome::ID " << AutomationMsg_ForwardContextMenuCommandToChrome::ID;
	LOG(INFO) << "AutomationMsg_TabReposition::ID " << AutomationMsg_TabReposition::ID;
	LOG(INFO) << "AutomationMsg_NavigateInExternalTab::ID " << AutomationMsg_NavigateInExternalTab::ID;
	LOG(INFO) << "AutomationMsg_NavigateExternalTabAtIndex::ID " << AutomationMsg_NavigateExternalTabAtIndex::ID;
	LOG(INFO) << "AutomationMsg_Find::ID " << AutomationMsg_Find::ID;
	LOG(INFO) << "AutomationMsg_SetInitialFocus::ID " << AutomationMsg_SetInitialFocus::ID;
	LOG(INFO) << "AutomationMsg_SetPageFontSize::ID " << AutomationMsg_SetPageFontSize::ID;
	LOG(INFO) << "AutomationMsg_SetProxyConfig::ID " << AutomationMsg_SetProxyConfig::ID;
	LOG(INFO) << "AutomationMsg_Cut::ID " << AutomationMsg_Cut::ID;
	LOG(INFO) << "AutomationMsg_Copy::ID " << AutomationMsg_Copy::ID;
	LOG(INFO) << "AutomationMsg_Paste::ID " << AutomationMsg_Paste::ID;
	LOG(INFO) << "AutomationMsg_SelectAll::ID " << AutomationMsg_SelectAll::ID;
	LOG(INFO) << "AutomationMsg_ReloadAsync::ID " << AutomationMsg_ReloadAsync::ID;
	LOG(INFO) << "AutomationMsg_StopAsync::ID " << AutomationMsg_StopAsync::ID;
	LOG(INFO) << "AutomationMsg_PrintAsync::ID " << AutomationMsg_PrintAsync::ID;
	LOG(INFO) << "AutomationMsg_HandleUnused::ID " << AutomationMsg_HandleUnused::ID;
	LOG(INFO) << "AutomationMsg_HandleMessageFromExternalHost::ID " << AutomationMsg_HandleMessageFromExternalHost::ID;
	LOG(INFO) << "AutomationMsg_RequestStarted::ID " << AutomationMsg_RequestStarted::ID;
	LOG(INFO) << "AutomationMsg_RequestData::ID " << AutomationMsg_RequestData::ID;
	LOG(INFO) << "AutomationMsg_RequestEnd::ID " << AutomationMsg_RequestEnd::ID;
	LOG(INFO) << "AutomationMsg_SaveAsAsync::ID " << AutomationMsg_SaveAsAsync::ID;
	LOG(INFO) << "AutomationMsg_RemoveBrowsingData::ID " << AutomationMsg_RemoveBrowsingData::ID;
	LOG(INFO) << "AutomationMsg_OverrideEncoding::ID " << AutomationMsg_OverrideEncoding::ID;
	LOG(INFO) << "AutomationMsg_RunUnloadHandlers::ID " << AutomationMsg_RunUnloadHandlers::ID;
	LOG(INFO) << "AutomationMsg_SetZoomLevel::ID " << AutomationMsg_SetZoomLevel::ID;
	LOG(INFO) << "$ " << __FUNCTION__ << " msg: " << msg->type();*/
...
void ChromeFrameAutomationProxyImpl::OnChannelError() {
  LOG(ERROR) << "Automation server died";
  if (proxy_entry_) {
...
AutomationProxyCacheEntry::AutomationProxyCacheEntry(
  ...
  DCHECK(delegate);
  thread_.reset(new base::Thread(base::UTF16ToASCII(profile_name).c_str()));
  thread_->Start();
...
void AutomationProxyCacheEntry::CreateProxy(ChromeFrameLaunchParams* params,
  ...
  DVLOG(1) << "*** " << __FUNCTION__ << " About to Launch";
  bool result = chrome_launcher::CreateLaunchCommandLine(&command_line);
  DVLOG(1) << "*** " << __FUNCTION__ << " Launch Called - result: " << result;
  if ( result ) {
	DVLOG(1) << "*** " << __FUNCTION__ << " Launch Called - success";
    ...
    //need to find pepflashplayer.dll
    //look in the PepperFlash directory
    if( file_exists( "PepperFlash\\pepflashplayer.dll" ) ) {
        command_line->AppendSwitchNative(switches::kPpapiFlashPath,
                                   L"PepperFlash\\pepflashplayer.dll");
    }
    //if not there, look in the root directory
    else if( file_exists( "pepflashplayer.dll" ) ) {
        command_line->AppendSwitchNative(switches::kPpapiFlashPath,
                                   L"pepflashplayer.dll");
    }
    //if not there, sadly give up and let the user stick with adobe

    //command_line->AppendSwitchNative(switches::kPpapiFlashPath,
    //                               L"PepperFlash\\pepflashplayer.dll");

    //command_line->AppendSwitchNative(switches::kPpapiFlashVersion,
    //                                 L"11.9.900.170");

    command_line_string = command_line->GetCommandLineString();
  }
  DVLOG(1) << "*** " << __FUNCTION__ << " COMMAND LINE: " << command_line_string;

  automation_server_launch_start_time_ = base::TimeTicks::Now();

  if (command_line_string.empty() ||
	  !base::LaunchProcess(command_line_string, base::LaunchOptions()).IsValid()) {
    DVLOG(1) << "*** " << __FUNCTION__ << " AUTOMATION_LAUNCH_INVALID";
    ...
    if (launch_result_ == AUTOMATION_SUCCESS) {
	  DVLOG(1) << "Result: " << launch_result_ << " ChromeFrame.AutomationServerLaunchSuccessTime: " << delta;
      UMA_HISTOGRAM_TIMES(
          "ChromeFrame.AutomationServerLaunchSuccessTime", delta);
    } else {
      DVLOG(1) << "AUTOMATION_LAUNCH_RESULT_INVALID: " << AUTOMATION_LAUNCH_RESULT_INVALID;
	  DVLOG(1) << "Result: " << launch_result_ << " ChromeFrame.AutomationServerLaunchFailedTime: " << delta;
      UMA_HISTOGRAM_TIMES(
...
void ProxyFactory::GetAutomationServer(
  ...
  DVLOG(1) << __FUNCTION__ << "chromeframe.createproxy";
  ...
  *automation_server_id = entry.get();
}
...
bool ChromeFrameAutomationClient::Initialize(
  ...
  proxy_factory_->GetAutomationServer(static_cast<LaunchDelegate*>(this),
      chrome_launch_params, &automation_server_id_); //joek: Messed with this
  return true;
}
...
void ChromeFrameAutomationClient::FindInPage(const std::wstring& search_string,
  ...
  params.search_string = base::WideToUTF16(search_string);
  ...
  if (result == AUTOMATION_SUCCESS) {
    ...
    if (automation_server_ != NULL) {
      ...
      if (external_tab_cookie_ == 0) {
		...
		LOG(INFO) << __FUNCTION__ << ": sending CreateExternalTabComplete";
	  ...
	  LOG(INFO) << "$ " << __FUNCTION__ << " launch failed";
...
void ChromeFrameAutomationClient::InitializeComplete(
  ...
  if (result != AUTOMATION_SUCCESS) {
    LOG(INFO) << "InitializeComplete: failure " << result;
  } else {
  ...
  if (chrome_frame_delegate_) {
    if (result == AUTOMATION_SUCCESS) {
      ...
      chrome_frame_delegate_->OnAutomationServerReady();
		...
		LOG(INFO) << "$ " << __FUNCTION__ << " 7";
      std::string version;
	  if (automation_server_) {
		  LOG(INFO) << "$ " << __FUNCTION__ << " 8";
		  version = automation_server_->server_version();
	  }
	  LOG(INFO) << "$ " << __FUNCTION__ << " 9";
...
bool ChromeFrameAutomationClient::ProcessUrlRequestMessage(TabProxy* tab,
	...
	DVLOG(1) << "*** " << __FUNCTION__ << " msg: " << msg.type();
  ...
  switch (msg_type) {
    ...
    case AutomationMsg_RequestStart::ID:
      if (ui_thread || (url_fetcher_flags_ &
                           PluginUrlRequestManager::START_REQUEST_THREADSAFE)) { //joek: Messed with all of these cases - there was a 'parameter' parameter added to dispatch()... just passing another 'this' in an attempt.
        AutomationMsg_RequestStart::Dispatch(&msg, url_fetcher_, this, this,
    ...
    case AutomationMsg_RequestRead::ID:
      if (ui_thread || (url_fetcher_flags_ &
        ...
        AutomationMsg_RequestRead::Dispatch(&msg, url_fetcher_, this, this,
    ...
    case AutomationMsg_RequestEnd::ID:
      if (ui_thread || (url_fetcher_flags_ &
        ...
        AutomationMsg_RequestEnd::Dispatch(&msg, url_fetcher_, this, this,
    ...
    case AutomationMsg_DownloadRequestInHost::ID:
      if (ui_thread || (url_fetcher_flags_ &
        ...
        AutomationMsg_DownloadRequestInHost::Dispatch(&msg, url_fetcher_, this, this,
    ...
    case AutomationMsg_GetCookiesFromHost::ID:
      if (ui_thread || (url_fetcher_flags_ &
        ...
        AutomationMsg_GetCookiesFromHost::Dispatch(&msg, url_fetcher_, this, this,
    ...
    case AutomationMsg_SetCookieAsync::ID:
      if (ui_thread || (url_fetcher_flags_ &
                          PluginUrlRequestManager::COOKIE_REQUEST_THREADSAFE)) {
        AutomationMsg_SetCookieAsync::Dispatch(&msg, url_fetcher_, this, this,
