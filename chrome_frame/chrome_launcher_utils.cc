...
// |command_line|.
bool CreateChromeLauncherCommandLine(scoped_ptr<base::CommandLine>* command_line) {
  DCHECK(command_line);
  bool success = false;
  ...
  if (PathService::Get(base::FILE_MODULE, &module_path)) {
    ...
    if (base::PathExists(chrome_launcher)) {
      command_line->reset(new base::CommandLine(chrome_launcher));
      success = true;
    }
  }
...
bool CreateUpdateCommandLine(const std::wstring& update_command,
  ...
  if (CreateChromeLauncherCommandLine(command_line)) {
    ...
    (*command_line)->AppendArg(base::UTF16ToASCII(update_command));
    success = true;
  }
...
bool CreateLaunchCommandLine(scoped_ptr<base::CommandLine>* command_line) {
  DCHECK(command_line);

  // Shortcut for OS versions that don't need the integrity broker.
  if (base::win::GetVersion() < base::win::VERSION_VISTA) {
    command_line->reset(new base::CommandLine(GetChromeExecutablePath()));
    ...
    // from being launched.
    const base::CommandLine& additional_params =
