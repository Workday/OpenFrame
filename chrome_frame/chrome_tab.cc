...
void InitGoogleUrl() {
  ...
  url::IsStandard(kDummyUrl,
                       url::MakeRange(0, arraysize(kDummyUrl)));
}
  ...
  if (base::win::GetVersion() < base::win::VERSION_VISTA &&
    ...
    if (channel_name.find(L"dev") != base::string16::npos) {
      ...
      if (ret == ERROR_SUCCESS) {
        base::CommandLine run_once_cmd(chrome_launcher::GetChromeExecutablePath());
...
HRESULT SetupUserLevelHelper() {
  ...
  //joek: removed this, as I don't think we need to worry about non-admin installs.
  ...
  if (reg_result == ERROR_SUCCESS) {
    ...
    while (value_index < ua_key.GetValueCount()) {
      ...
      LRESULT ret = ::RegEnumValue(ua_key.Handle(), value_index, value_name,
                                   ...
                                   &value_size);
      if (ret == ERROR_SUCCESS) {
        if (base::StartsWith(value_name, kChromeFramePrefix, base::CompareCase::INSENSITIVE_ASCII)) {
...
class SecurityDescBackup {
 ...
 private:
  ...
  bool ReadBackupKey(std::wstring* sddl) {
    ...
    if (backup_key.ReadValue(NULL, base::WriteInto(sddl, wchar_count), &len,
...
HRESULT MuxErrorIntoHRESULT(HRESULT hr, int error_code) {
  ...
  //joek: I can't find this defined anywhere...:
