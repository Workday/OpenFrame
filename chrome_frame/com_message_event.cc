...
STDMETHODIMP ComMessageEvent::GetIDsOfNames(REFIID iid, LPOLESTR* names,
  ...
  for (UINT i = 0; SUCCEEDED(hr) && i < count_names; ++i) {
	...
	const base::StringPiece16 name_combo = base::StringPiece16(names[i]);//joek: Changed this around - let's hope it works right (original lines above)
	DVLOG(1) << __FUNCTION__ << " " << names[i];
	if (base::LowerCaseEqualsASCII(name_combo, "data")) {
      dispids[i] = DISPID_MESSAGE_EVENT_DATA;
    } else if (base::LowerCaseEqualsASCII(name_combo, "origin")) {
      dispids[i] = DISPID_MESSAGE_EVENT_ORIGIN;
    } else if (base::LowerCaseEqualsASCII(name_combo, "lasteventid")) {
      dispids[i] = DISPID_MESSAGE_EVENT_LAST_EVENT_ID;
    } else if (base::LowerCaseEqualsASCII(name_combo, "source")) {
      dispids[i] = DISPID_MESSAGE_EVENT_SOURCE;
    } else if (base::LowerCaseEqualsASCII(name_combo, "messageport")) {
      dispids[i] = DISPID_MESSAGE_EVENT_MESSAGE_PORT;
    } else if (base::LowerCaseEqualsASCII(name_combo, "type")) {
