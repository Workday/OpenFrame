...
DllRedirector::DllRedirector(const char* shared_memory_name)
  ...
  shared_memory_.reset(
	  new base::SharedMemory(base::ASCIIToUTF16(shared_memory_name)));
}
...
DllRedirector* DllRedirector::GetInstance() {
  return base::Singleton<DllRedirector>::get();
}
...
bool DllRedirector::SetFileMappingToReadOnly(HANDLE mapping) {
  bool success = false;
...
void DllRedirector::Lock() {
	if (lock_ == NULL) {
		std::wstring name = base::ASCIIToUTF16(shared_memory_name_);
		name.append(L"lock");
		LOG(INFO) << "Creating Lock: " << name;
...
bool DllRedirector::RegisterAsFirstCFModule() {
  ...
  bool result = shared_memory_->CreateNamedDeprecated(shared_memory_name_.c_str(),
                                            false,  // open_existing
                                            kSharedMemorySize);
  ...
  if (result) {
    ...
    if (!SetFileMappingToReadOnly(shared_memory_->handle().GetHandle())) {
...
Version* DllRedirector::GetCurrentModuleVersion() {
  ...
  if (file_version_info.get()) {
     current_version.reset(
         new Version(base::UTF16ToASCII(file_version_info->file_version())));
...
HMODULE DllRedirector::LoadVersionedModule(Version* version) {
  ...
  if (GetModuleFileName(this_module, system_buffer,
    ...
    module_path = module_path.DirName()
                             .DirName()
							 .Append(base::ASCIIToUTF16(version->GetString()))
                             .Append(module_name);
