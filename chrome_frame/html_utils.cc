...
bool HTMLScanner::StringRange::LowerCaseEqualsASCII(const char* other) const {
	return base::LowerCaseEqualsASCII(base::StringPiece16(start_, end_), other); //joek: messed with this
}
...
bool HTMLScanner::StringRange::GetTagName(std::wstring* tag_name) const {
  ...
  name_start++;
  while (name_start < end_ && base::IsUnicodeWhitespace(*name_start))
    name_start++;
  ...
  while (name_end < end_ && !base::IsUnicodeWhitespace(*name_end))
    name_end++;
...
bool HTMLScanner::StringRange::GetTagAttribute(const wchar_t* attribute_name,
  ...
  // from string_util.h.
  std::string search_name_ascii(base::UTF16ToASCII(attribute_name));
  ...
  while (tokenizer.GetNext()) {
    ...
    if (PARSE_STATE_NAME == parse_state) {
      // We have a tag name, check to see if it matches our target name:
		if (base::LowerCaseEqualsASCII(base::StringPiece16(tokenizer.token_begin(), tokenizer.token_end()), //joek: messed with this
...
HTMLScanner::HTMLScanner(const wchar_t* html_string)
    : html_string_(base::CollapseWhitespace(html_string, true)),
      quotes_(kQuotes) {
}
...
void HTMLScanner::GetTagsByName(const wchar_t* name, StringRangeList* tag_list,
  ...
  base::TrimWhitespace(search_name, base::TRIM_ALL, &search_name);

  // Use this so we can use the convenience method LowerCaseEqualsASCII()
  // from string_util.h.
  std::string search_name_ascii(base::UTF16ToASCII(search_name));
  std::string stop_tag_ascii(base::UTF16ToASCII(stop_tag));
  ...
  while (NextTag(&remaining_html, &current_tag)) {
    if (current_tag.GetTagName(&current_name)) {
      if (base::LowerCaseEqualsASCII(current_name, search_name_ascii.c_str())) {
        tag_list->push_back(current_tag);
      } else if (base::LowerCaseEqualsASCII(current_name, stop_tag_ascii.c_str())) {
...
//joek: this was removed - so took it from string_util.cc:528e265
bool HTMLScanner::StartsWith(const base::string16& str,
	...
	return base::StartsWith(base::StringPiece16(str), base::StringPiece16(search),
...
const char* GetChromeUserAgent() {
  if (!g_chrome_user_agent[0]) {
    ...
    if (!g_chrome_user_agent[0]) {
      ...
      std::string product("Chrome/"); //joek: modified this
	  //product += version_info::is_valid() ? version_info::GetVersionNumber()
      //                                   : "0.0.0.0";
	  product += version_info::GetVersionNumber();

      ua = content::BuildUserAgentFromProduct(product);
...
std::string GetDefaultUserAgent() {
  ...
  for (int retries = 1; hr == E_OUTOFMEMORY && retries <= 10; ++retries) {
    hr = ::ObtainUserAgentString(0, base::WriteInto(&ret, size + 1), &size);
    if (hr == E_OUTOFMEMORY) {
