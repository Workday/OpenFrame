...
std::string GetUserAgentFromHeaders(LPCWSTR headers,
  ...
  if (additional_headers) {
    ascii_headers = base::UTF16ToASCII(additional_headers);
  }
  ...
  if (headers_iterator.AdvanceTo(kLowerCaseUserAgent)) {
    ...
    std::string original_headers(base::UTF16ToASCII(headers));
...
std::string ExcludeFieldFromHeaders(const std::string& old_headers,
  ...
  while (headers_iterator.GetNext()) {
    if (!base::LowerCaseEqualsASCII(base::string16(headers_iterator.name_begin(),
...
std::string ReplaceOrAddUserAgent(LPCWSTR headers,
  ...
  std::string new_headers;
  if (headers) {
    std::string ascii_headers(base::UTF16ToASCII(headers));
...
bool HttpNegotiatePatch::Initialize() {
  ...
  if (bind_ctx) {
    ...
    if (bscb_holder) {
      hr = PatchHttpNegotiate(bscb_holder.get());
    } else {
...
HRESULT HttpNegotiatePatch::PatchHttpNegotiate(IUnknown* to_patch) {
  ...
  if (http) {
    hr = vtable_patch::PatchInterfaceMethods(http.get(), IHttpNegotiate_PatchInfo);
...
HRESULT HttpNegotiatePatch::BeginningTransaction(
  ...
  if (modify_user_agent_) {
    ...
    *additional_headers = reinterpret_cast<wchar_t*>(::CoTaskMemRealloc(
    ...
    lstrcpyW(*additional_headers, base::ASCIIToUTF16(updated_headers).c_str());
  } else {
