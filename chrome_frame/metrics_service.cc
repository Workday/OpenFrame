...
class ChromeFrameMetricsDataUploader : public BSCBImpl {
 public:
  ...
  HRESULT UploadData(const std::string& upload_data) {
    ...
    RewindStream(cache_stream_.get());
	std::wstring server_url = base::ASCIIToUTF16(::metrics::kDefaultMetricsServerUrl);

	hr = CreateURLMoniker(NULL, server_url.c_str(),
  ...
  STDMETHOD(BeginningTransaction)(LPCWSTR url, LPCWSTR headers, DWORD reserved,
	...
	std::wstring mime_type = base::ASCIIToUTF16(::metrics::kDefaultMetricsMimeType);
...
const std::string& MetricsService::GetClientID() {
  ...
  if (client_id_.empty()) {
    ...
    int result = StringFromGUID2(guid,
                                 base::WriteInto(&guid_string, kGUIDSize), kGUIDSize);
...
void MetricsService::StartRecording() {
  DCHECK_EQ(thread_, base::PlatformThread::CurrentId());
  if (log_manager()->current_log())
    return;

  metrics::MetricsLog::LogType log_type = (state_ == INITIALIZED) ?
      metrics::MetricsLog::INITIAL_STABILITY_LOG : metrics::MetricsLog::ONGOING_LOG;
  log_manager()->BeginLoggingWithLog(new metrics::MetricsLog(GetClientID(),
                                                      session_id_,
                                                      log_type,
                                                      GetVersionString()));
...
void MetricsService::StopRecording(bool save_log) {
  ...
  if (save_log) {
    ...
    //RecordCurrentHistograms(); //joek: removed this... just easier.
  }
