...
PluginUrlRequest::PluginUrlRequest()
    : delegate_(NULL),
	  ...
	  resource_type_(content::ResourceType::RESOURCE_TYPE_MAIN_FRAME),
      load_flags_(0),
      is_chunked_upload_(false) {
	DVLOG(1) << "* " << __FUNCTION__ << " Constructor ";
}
...
bool PluginUrlRequest::Initialize(PluginUrlRequestDelegate* delegate,
	...
	net::UploadData* upload_data, content::ResourceType resource_type,
    bool enable_frame_busting, int load_flags) {
  DVLOG(1) << "* " << __FUNCTION__ << " " << delegate;
