...
STDMETHODIMP ProtocolSinkWrap::ReportProgress(ULONG status_code,
  ...
  HRESULT hr = prot_data_->ReportProgress(delegate_.get(), status_code, status_text);
  return hr;
}
...
STDMETHODIMP ProtocolSinkWrap::ReportData(DWORD flags, ULONG progress,
  ...
  HRESULT hr = prot_data_->ReportData(delegate_.get(), flags, progress, max_progress);
  return hr;
}
...
STDMETHODIMP ProtocolSinkWrap::ReportResult(HRESULT result, DWORD error,
  ...
  HRESULT hr = prot_data_->ReportResult(delegate_.get(), result, error, result_text);
  return hr;
}
...
bool ShouldWrapSink(IInternetProtocolSink* sink, const wchar_t* url) {
  ...
  DoQueryService(GUID_NULL, sink, http_negotiate.Receive());
  if (http_negotiate && !IsSubFrameRequest(http_negotiate.get()))
    return true;
...
bool IsTextHtml(const wchar_t* status_text) {
  ...
  bool is_text_html = base::LowerCaseEqualsASCII(str, kTextHtmlMimeType);
  return is_text_html;
}
...
bool IsAdditionallySupportedContentType(const wchar_t* status_text) {
  ...
  for (int i = 0; i < arraysize(kHeaderContentTypes); ++i) {
    if (base::LowerCaseEqualsASCII(str, kHeaderContentTypes[i]))
      return true;
  }
...
RendererType DetermineRendererType(void* buffer, DWORD size, bool last_chance) {
  ...
  if (SUCCEEDED(UtilGetXUACompatContentValue(html_contents,
                                             &xua_compat_content))) {
    if (CheckXUaCompatibleDirective(base::UTF16ToASCII(xua_compat_content),
                                    GetIEMajorVersion())) {
...
bool HandleAttachToExistingExternalTab(LPCWSTR url,
  ...
  if (cf_url.Parse(url) && cf_url.attach_to_external_tab()) {
    ...
    if (!prot_data) {
      ...
      PutProtData(bind_ctx, prot_data.get());
    }
...
STDMETHODIMP Hook_Start(InternetProtocol_Start_Fn orig_start,
  ...
  scoped_refptr<ProtData> prot_data = ProtData::DataFromProtocol(protocol);
  if (prot_data && !HasProtData(bind_ctx.get())) {
    prot_data->Invalidate();
    prot_data = NULL;
  }

  if (HandleAttachToExistingExternalTab(url, protocol, prot_sink, bind_ctx.get())) {
    return S_OK;
  }

  if (IsCFRequest(bind_ctx.get())) {
    base::win::ScopedComPtr<BindContextInfo> info;
    BindContextInfo::FromBindContext(bind_ctx.get(), info.Receive());
    DCHECK(info);
    if (info) {
  ...
  if (prot_data) {
    ...
    base::win::ScopedComPtr<IInternetProtocolSink> new_sink =
        ProtocolSinkWrap::CreateNewSink(prot_sink, prot_data.get());
    return ForwardWrappedHookStart(orig_start, protocol, url, new_sink.get(),
  ...
  PutProtData(bind_ctx.get(), prot_data.get());

  base::win::ScopedComPtr<IInternetProtocolSink> new_sink =
      ProtocolSinkWrap::CreateNewSink(prot_sink, prot_data.get());
  return ForwardWrappedHookStart(orig_start, protocol, url, new_sink.get(), bind_info,
                                 flags, reserved);
}
...
STDMETHODIMP Hook_StartEx(InternetProtocol_StartEx_Fn orig_start_ex,
  ...
  if (prot_data) {
    ...
    base::win::ScopedComPtr<IInternetProtocolSink> new_sink =
        ProtocolSinkWrap::CreateNewSink(prot_sink, prot_data.get());
    return ForwardWrappedHookStartEx(orig_start_ex, protocol, uri, new_sink.get(),
                                     bind_info, flags, reserved);
  }
...
void TransactionHooks::InstallHooks() {
  ...
  do {
    ...
    hr = mk->BindToStorage(bc.get(), NULL, IID_IStream,
  ...
  if (prot.transaction_) {
    HookTransactionVtable(prot.transaction_.get());
