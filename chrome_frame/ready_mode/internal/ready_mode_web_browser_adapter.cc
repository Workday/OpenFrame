...
bool ReadyModeWebBrowserAdapter::Initialize(IWebBrowser2* web_browser,
  ...
  HRESULT hr = DispEventAdvise(web_browser_.get(), &DIID_DWebBrowserEvents2);
  if (FAILED(hr)) {
    observer_.reset();
...
void ReadyModeWebBrowserAdapter::Uninitialize() {
  ...
  HRESULT hr = DispEventUnadvise(web_browser_.get(), &DIID_DWebBrowserEvents2);
  if (FAILED(hr)) {
...
STDMETHODIMP_(void) ReadyModeWebBrowserAdapter::DocumentComplete(
  ...
  chrome_frame_doc.QueryFrom(doc_disp.get());
