...
bool IsInvalidTagCharacter(wchar_t tag_character) {
  return !(L'-' == tag_character ||
           base::IsAsciiDigit(tag_character) ||
           base::IsAsciiAlpha(tag_character));
}
...
class CompareInsensitiveASCII
 ...
 public:
	 explicit CompareInsensitiveASCII(const base::string16& value)
	  //: value_lowered_(base::UTF16ToASCII(value)) {
	  : value_lowered_(base::UTF16ToUTF8(value)) {
	  value_lowered_ = base::ToLowerASCII(value_lowered_);
  }
  bool operator()(const base::string16& comparand) {
	  return base::LowerCaseEqualsASCII(comparand, value_lowered_.c_str());
  }
...
bool PushBackIfAbsent(
    ...
    std::vector<base::string16>* collection) {
  if (collection->end() ==
...
bool PushBackWithFallbackIfAbsent(
  ...
  if (!language.empty()) {
    ...
    base::string16::size_type dash_pos = language.find(L'-');
...
SimpleResourceLoader::SimpleResourceLoader()
  ...
  std::vector<base::string16> language_tags;
...
SimpleResourceLoader* SimpleResourceLoader::GetInstance() {
  return base::Singleton<SimpleResourceLoader>::get();
}
...
void SimpleResourceLoader::GetPreferredLanguages(
  ...
  if (base::win::i18n::GetThreadPreferredUILanguageList(&languages)) {
    for (std::vector<base::string16>::const_iterator scan = languages.begin(),
  ...
  PushBackWithFallbackIfAbsent(base::ASCIIToUTF16(
...
bool SimpleResourceLoader::IsValidLanguageTag(
    const base::string16& language_tag) {
...
bool SimpleResourceLoader::LoadLocalePack(
    const std::vector<base::string16>& language_tags,
  ...
  const base::string16 dll_suffix(L".dll");
  const base::string16 pack_suffix(L".pak");

  bool found_pack = false;

  for (std::vector<base::string16>::const_iterator scan = language_tags.begin(),
...
base::string16 SimpleResourceLoader::GetLocalizedResource(int message_id) {
  if (!data_pack_) {
...
// static
base::string16 SimpleResourceLoader::GetLanguage() {
  return SimpleResourceLoader::GetInstance()->language_;
}

// static
base::string16 SimpleResourceLoader::Get(int message_id) {
