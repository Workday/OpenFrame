...
void BrowserObserver::ShowPrompt() {
  ...
  if (infobar_manager) {
    ...
    scoped_ptr<UrlLauncher> url_launcher(new UrlLauncherImpl(web_browser_.get()));
    ...
    scoped_ptr<InfobarContent> infobar_content(new TurndownPromptContent(
        ...
        base::Bind(&OnUninstallClicked,
                   base::Owned(new UrlLauncherImpl(web_browser_.get())))));
...
InfobarManager* BrowserObserver::GetInfobarManager() {
  ...
  hr = DoQueryService(SID_SShellBrowser, web_browser_.get(), ole_window.Receive());
...
bool IsCurrentModuleSystemLevel() {
  ...
  if (PathService::Get(base::DIR_MODULE, &dll_path))
    return !InstallUtil::IsPerUserInstall(dll_path);
  return false;
}
...
bool InstallPrompts(IWebBrowser2* web_browser) {
  ...
  scoped_ptr<ReadyModeWebBrowserAdapter::Observer> browser_observer(
      new BrowserObserver(web_browser, adapter.get()));
...
void LaunchChromeFrameUninstaller() {
  ...
  base::CommandLine uninstall_command(product_state.uninstall_command());
  ...
  //joek: we don't need to call this: base::LaunchProcess(uninstall_command, base::LaunchOptions());
}
