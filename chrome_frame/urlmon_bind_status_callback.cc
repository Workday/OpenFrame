...
HRESULT SniffData::DrainCache(IBindStatusCallback* bscb, DWORD bscf,
  ...
  // last call
  HGLOBAL memory = NULL;
  HRESULT hr = GetHGlobalFromStream(cache_.get(), &memory);
  ...
  if (is_undetermined()) {
    ...
    if (IsChrome(RendererTypeForUrl(url_))) {
      ...
      if (is_cache_valid() && cache_) {
        HGLOBAL memory = NULL;
        GetHGlobalFromStream(cache_.get(), &memory);
...
HRESULT BSCBStorageBind::MayPlayBack(DWORD flags) {
  ...
  if (data_sniffer_.is_cache_valid()) {
    if (data_sniffer_.is_chrome()) {
      ...
      BindContextInfo::FromBindContext(bind_ctx_.get(), info.Receive());
      DCHECK(info);
      if (info) {
        info->SetToSwitch(data_sniffer_.cache_.get());
      }
    }
