    ...
    if (SUCCEEDED(hr)) {
      ...
      if (!referrer_.empty()) {
        headers = base::StringPrintf(L"Referer: %ls\r\n\r\n",
            base::ASCIIToUTF16(referrer_).c_str());
      }
      ...
      hr = NavigateBrowserToMoniker(browser, moniker.get(), headers.c_str(),
          bind_context.get(), fragment.c_str(), NULL, &flags);
...
bool MonikerPatch::Initialize() {
  ...
  DCHECK(SUCCEEDED(hr));
  if (SUCCEEDED(hr)) {
    hr = vtable_patch::PatchInterfaceMethods(moniker.get(), IMoniker_PatchInfo);
...
bool ShouldWrapCallback(IMoniker* moniker, REFIID iid, IBindCtx* bind_context) {
  ...
  if (S_OK == bind_context->GetObjectParam(L"_BSCB_Holder_",
    ...
    if (S_OK != DoQueryService(IID_IBindStatusCallback, bscb_holder.get(),
                               bscb.Receive()))
      return false;
    ...
    if (S_OK != bind_info.QueryFrom(bscb.get()))
      return false;
  }
