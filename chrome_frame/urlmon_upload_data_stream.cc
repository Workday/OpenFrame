...
net::UploadDataStream* CreateUploadDataStream(net::UploadData* upload_data) {
  ...
  if (upload_data->is_chunked()) {
	...
	net::ChunkedUploadDataStream* chunkedPointer = new net::ChunkedUploadDataStream(upload_data->identifier());
	upload_data_stream = chunkedPointer;
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
	  ...
	  chunkedPointer->AppendData(element.bytes(), element.bytes_length(),
                                      is_last_chunk);
    }
  } else {
    // Not chunked.
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
      ...
      DCHECK(reader);
	  element_readers.push_back(make_scoped_ptr(reader));
    }
	upload_data_stream = new net::ElementsUploadDataStream(std::move(element_readers), //joek: changed this around a bit
...
STDMETHODIMP UrlmonUploadDataStream::Read(void* pv, ULONG cb, ULONG* read) {
  ...
  while (bytes_copied < total_bytes_to_copy) {
    ...
    int bytes_read = request_body_stream_->Read(buf.get(), buf->size(),
...
STDMETHODIMP UrlmonUploadDataStream::Seek(LARGE_INTEGER move, DWORD origin,
  ...
  if (origin == STREAM_SEEK_SET && move.QuadPart == 0) {
    if (request_body_stream_->position() != 0) {
      request_body_stream_.reset(CreateUploadDataStream(upload_data_.get()));
