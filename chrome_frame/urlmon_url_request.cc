...
void UrlmonUrlRequest::Stop() {
  ...
  DVLOG(1) << "*Delegate Stuff " << __FUNCTION__ << " " << delegate_;
  delegate_ = NULL;
...
void UrlmonUrlRequest::TerminateBind(const TerminateBindCallback& callback) {
  ...
  if (status_.get_state() == Status::DONE) {
    ...
    callback.Run(moniker_.get(), bind_context_.get(), upload_data_.get(),
    ...
    if (pending_data_) {
      ...
      HRESULT hr;
      while ((hr = ReadStream(read_stream.get(), 0xffff, &data)) == S_OK) {
        // Just drop the data.
      }
...
size_t UrlmonUrlRequest::SendDataToDelegate(size_t bytes_to_read) {
  ...
  if (delegate_) {
    ...
    if (cache_) {
      HRESULT hr = ReadStream(cache_.get(), bytes_to_read, &read_data);
    ...
    if (read_data.empty() && pending_data_) {
      ...
      HRESULT hr = ReadStream(pending.get(), bytes_to_read, &read_data);
      if (read_data.empty())
...
STDMETHODIMP UrlmonUrlRequest::OnStartBinding(DWORD reserved,
  ...
  binding_ = binding;
  if (pending_) {
    response_headers_ = GetHttpHeadersFromBinding(binding_.get());
...
STDMETHODIMP UrlmonUrlRequest::OnProgress(ULONG progress, ULONG max_progress,
  ...
  DVLOG(1) << __FUNCTION__ << " Testing " << delegate_;
  if (!delegate_) {
  ...
  switch (status_code) {
    ...
    case BINDSTATUS_REDIRECTING: {
      ...
      BindContextInfo::FromBindContext(bind_context_.get(), info.Receive());
      DCHECK(info);
      ...
      if (GURL(status_text) != previously_redirected) {
        ...
        int http_code = GetHttpResponseStatusFromBinding(binding_.get());
...
STDMETHODIMP UrlmonUrlRequest::OnStopBinding(HRESULT result, LPCWSTR error) {
  ...
  if (result == INET_E_TERMINATED_BIND) {
    if (terminate_requested()) {
      terminate_bind_callback_.Run(moniker_.get(), bind_context_.get(), upload_data_.get(),
                                   request_headers_.c_str());
    } else {
  ...
  if (state == Status::WORKING) {
    ...
    if (FAILED(result)) {
      ...
      if (IS_HTTP_SUCCESS_CODE(http_code)) {
        ...
        std::string headers = GetHttpHeadersFromBinding(binding_.get());
  ...
  *bind_flags = BINDF_ASYNCHRONOUS | BINDF_ASYNCSTORAGE | BINDF_PULLDATA;
  ...
  if (base::LowerCaseEqualsASCII(method(), "get")) {
    ...
    bind_info->dwBindVerb = BINDVERB_PUT;
	  ...
	  std::wstring verb(base::ASCIIToUTF16(base::ToUpperASCII(method())));
  ...
  if (bind_info->dwBindVerb == BINDVERB_POST ||
    ...
    *bind_flags |= BINDF_GETNEWESTVERSION | BINDF_PRAGMA_NO_CACHE;
	...
	if (resource_type_ != content::ResourceType::RESOURCE_TYPE_MAIN_FRAME)
  ...
  if (!new_headers.empty()) {
    *additional_headers = reinterpret_cast<wchar_t*>(
    ...
    if (*additional_headers == NULL) {
      ...
      lstrcpynW(*additional_headers, base::ASCIIToUTF16(new_headers).c_str(),
                new_headers.size());
    }
  }
...
STDMETHODIMP UrlmonUrlRequest::OnResponse(DWORD dwResponseCode,
  ...
  if (response_headers)
    raw_headers = base::WideToUTF8(response_headers);
...
HRESULT UrlmonUrlRequest::StartAsyncDownload() {
  ...
  if (bind_context_.get() == NULL)  {
    ...
    hr = ::RegisterBindStatusCallback(bind_context_.get(), this, NULL, 0);
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = moniker_->BindToStorage(bind_context_.get(), NULL, __uuidof(IStream),
...
void UrlmonUrlRequest::ReleaseBindings() {
  ...
  // to be available.
  if (bind_context_)
    ::RevokeBindStatusCallback(bind_context_.get(), this);
}
...
void UrlmonUrlRequestManager::StartRequest(int request_id,
  ...
  if (background_worker_thread_enabled_ &&
	  ...
	  static_cast<content::ResourceType>(request_info.resource_type))) {
...
void UrlmonUrlRequestManager::StartRequestHelper(
  ...
  DVLOG(1) << "* " << __FUNCTION__ << " StartRequestHelper ";
  DCHECK(request_map);
  ...
  DVLOG(1) << "* " << __FUNCTION__ << " Call Initialize ";
  ...
  if (!is_started) {
    ...
    new_request->OnResponse(
        0, base::UTF8ToWide(new_request->response_headers()).c_str(), NULL, NULL);
  }
}
...
void UrlmonUrlRequestManager::DownloadRequestInHost(int request_id) {
  ...
  scoped_refptr<UrlmonUrlRequest> request(LookupRequest(request_id,
                                                        &request_map_));
  if (request) {
    DownloadRequestInHostHelper(request.get());
  } else if (background_worker_thread_enabled_) {
    base::AutoLock lock(background_resource_map_lock_);
    request = LookupRequest(request_id, &background_request_map_);
    if (request) {
...
void UrlmonUrlRequestManager::GetCookiesForUrl(const GURL& url, int cookie_id) {
  ...
  InternetGetCookieA(url.spec().c_str(), NULL, NULL,
                                   &cookie_size);
  DWORD error = 0;
  if (cookie_size) {
