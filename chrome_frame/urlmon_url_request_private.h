  ...
  class Status {
   public:
    ...
    void set_result(net::URLRequestStatus::Status status, int error) {
      //result_.set_status(status); //joek
      //result_.set_error(error);
	  result_ = net::URLRequestStatus(status, error);
    }
    ...
    void set_result(HRESULT hr) {
      ...
      //result_.set_error(HresultToNetError(hr));
		result_ = net::URLRequestStatus(FAILED(hr) ? net::URLRequestStatus::FAILED : net::URLRequestStatus::SUCCESS,
