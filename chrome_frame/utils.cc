...
HRESULT UtilGetXUACompatContentValue(const std::wstring& html_string,
  ...
  if (base::ToLowerASCII(html_string).find(kXUACompatValue) ==
      std::wstring::npos) {
    return E_FAIL;
  }
  ...
  std::string search_attribute_ascii(base::UTF16ToASCII(kXUACompatValue));
...
void DisplayVersionMismatchWarning(HWND parent,
  ...
  if (server_version.empty()) {
    ...
    wide_server_version = base::ASCIIToUTF16(server_version);
  }
...
BrowserType GetBrowserType() {
  ...
  if (browser_type == BROWSER_INVALID) {
    ...
    if (!exe.empty()) {
	  ...
	  if (base::LowerCaseEqualsASCII(base::string16(begin, end), kIEImageName)) {
...
uint32 GetIEMajorVersion() {
  ...
  if (ie_major_version == UINT_MAX) {
	...
	DVLOG(1) << "*GetIEMajorVersion handle: " << exe_name;
    if (!base::LowerCaseEqualsASCII(exe_name, kIEImageName)) {
		DVLOG(1) << "*GetIEMajorVersion not found1";
      ie_major_version = 0;
    } else {
      uint32 high = 0;
      uint32 low  = 0;
      if (GetModuleVersion(mod, &high, &low)) {
		...
		DVLOG(1) << "*GetIEMajorVersion found: " << ie_major_version;
      } else {
		  DVLOG(1) << "*GetIEMajorVersion not found2";
        ie_major_version = 0;
      }
    }
  }
...
IEVersion GetIEVersion() {
  ...
  if (ie_version == IE_INVALID) {
	...
	DVLOG(1) << "*GetIEMajorVersion: " << major_version;
...
HRESULT NavigateBrowserToMoniker(IUnknown* browser, IMoniker* moniker,
  ...
  if (uri_container) {
    ...
    if (browser_priv2) {
      ...
      if (GetIEVersion() < IE_9) {
        hr = browser_priv2->NavigateWithBindCtx2(
                uri_obj.get(), flags, NULL, post_data_variant.AsInput(),
    ...
    if (SUCCEEDED(hr = moniker->GetDisplayName(bind_ctx, NULL, &url))) {
      ...
      if (SUCCEEDED(hr = browser_priv.QueryFrom(web_browser2.get()))) {
        GURL target_url(url);
...
std::wstring GetActualUrlFromMoniker(IMoniker* moniker,
  ...
  if (base::StartsWith(bho_url, moniker_url, base::CompareCase::INSENSITIVE_ASCII) &&
...
std::wstring GuidToString(const GUID& guid) {
  std::wstring ret;
  ::StringFromGUID2(guid, base::WriteInto(&ret, 39), 39);
  return ret;
}
...
std::string GetHttpHeadersFromBinding(IBinding* binding) {
  ...
  return GetRawHttpHeaders(info.get());
}
...
HRESULT ReadStream(IStream* stream, size_t size, std::string* data) {
  ...
  DWORD read = 0;
  HRESULT hr = stream->Read(base::WriteInto(data, size + 1), size, &read);
...
bool ChromeFrameUrl::ParseAttachExternalTabUrl() {
  ...
  if (!base::StartsWith(query, kAttachExternalTabPrefix, base::CompareCase::INSENSITIVE_ASCII)) {
...
int GetXUaCompatibleDirective(const std::string& directive, char delimiter) {
  ...
  while (name_value_pairs.GetNext()) {
	  if (!LowerCaseEqualsASCII(base::StringPiece(name_value_pairs.name_begin(),
    ...
    if (filter_length < 3 ||
        !base::LowerCaseEqualsASCII(base::StringPiece(filter_begin, filter_begin + 2), "ie") ||
...
bool IsChromeFrameDocument(IWebBrowser2* web_browser) {
  ...
  if (doc) {
    ...
    chrome_frame.QueryFrom(doc.get());
