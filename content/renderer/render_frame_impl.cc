...
WebNavigationPolicy RenderFrameImpl::decidePolicyForNavigation(
	...
	LOG(WARNING) << "@@Questionable " << __FUNCTION__ << " " << info.urlRequest.url();
  ...
  if (is_content_initiated) {
	  LOG(WARNING) << "@@Questionable " << __FUNCTION__;
	  bool is_form_post = ((info.navigationType == blink::WebNavigationTypeFormSubmitted) ||
		  (info.navigationType == blink::WebNavigationTypeFormResubmitted)) &&
		  base::EqualsASCII(base::UTF8ToWide(info.urlRequest.httpMethod().utf8()), "POST");
	  LOG(WARNING) << "@@Questionable " << is_form_post << "  " << render_view_->renderer_preferences_.browser_handles_non_local_top_level_requests;
	  bool browser_handles_request =
		  render_view_->renderer_preferences_
		  .browser_handles_non_local_top_level_requests
		  && IsNonLocalTopLevelNavigation(url, frame_, info.navigationType, is_form_post);
	  if (!browser_handles_request) {
		  browser_handles_request = IsTopLevelNavigation(frame_) &&
			  render_view_->renderer_preferences_
			  .browser_handles_all_top_level_requests;
	  }
	  if (browser_handles_request) {
		  LOG(WARNING) << "@@Questionable " << "Handling Request";
  ...
  if (is_content_initiated && IsTopLevelNavigation(frame_) &&
      render_view_->renderer_preferences_
          .browser_handles_all_top_level_requests) {
	  LOG(WARNING) << "@@Questionable " << "Open1";
    OpenURL(url, referrer, info.defaultPolicy, info.replacesCurrentHistoryItem,
            false);
	LOG(WARNING) << "@@Questionable " << "Return1";
    return blink::WebNavigationPolicyIgnore;  // Suppress the load here.
  }
  ...
  if (SiteIsolationPolicy::UseSubframeNavigationEntries() &&
	  ...
	  LOG(WARNING) << "@@Questionable " << "Open2";
    OpenURL(url, referrer, info.defaultPolicy, info.replacesCurrentHistoryItem,
            true);
    // Suppress the load in Blink but mark the frame as loading.
	LOG(WARNING) << "@@Questionable " << "Return2";
  ...
  if (!frame_->parent() && is_content_initiated &&
    ...
    if (should_fork) {
		LOG(WARNING) << "@@Questionable " << "Open3";
      OpenURL(url, send_referrer ? referrer : Referrer(), info.defaultPolicy,
              info.replacesCurrentHistoryItem, false);
	  LOG(WARNING) << "@@Questionable " << "Return3";
      return blink::WebNavigationPolicyIgnore;  // Suppress the load here.
    }
	LOG(WARNING) << "@@Questionable " << "Return4";
  }
