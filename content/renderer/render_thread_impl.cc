...
bool RenderThreadImpl::Send(IPC::Message* msg) {
  ...
  if (msg->is_sync()) {
    if (msg->is_caller_pumping_messages()) {
      pumping_events = true;
	  LOG(INFO) << "$ PUMPING_EVENTS IS TRUE - 1";
    } else {  //joek: messing with this - at some point it was removed, not sure if it's needed for ChromeFrame, but we'll have to put back in the ViewHostMsg_GetCookies (etc) if it is
		LOG(INFO) << "$ " << __FUNCTION__ << " 1 " << msg->type();
      if (/*(msg->type() == ViewHostMsg_GetCookies::ID ||
		  ...
		  LOG(INFO) << "$ PUMPING_EVENTS IS TRUE - 2";
      }
